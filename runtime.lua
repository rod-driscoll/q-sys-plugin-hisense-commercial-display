...
local InputTypes = {
  ...
  {Name="HDMI 1"      , Tx='\x0E', Rx={ '\x05\x03\x04', '\x05\x06', '\x0E' } },
  {Name="HDMI 2"      , Tx='\x0F', Rx={ '\x05\x03\x03', '\x05\x07', '\x0F' } },
...
if Controls then
  ...
  -- Control aliases
    ...
    DebugPrint = Properties["Debug Print"].Value
  	local DisplaySeries = Properties["Display Series"].Value == "Auto" and "BM" or Properties["Display Series"].Value
    ...
    local Request = {
    	...
    	Erase       ={Command=0x10,Data=''},           -- DD FF 00 06 C1 10 00 00 01 D6 BB CC -- this is documented as the PanelStatus command, but erases settings such as WoL
  ...
  --[[ Protocols compiled from several documents. Not all is implemented by either this plugin or the actual devices
  	...
  	local InputProtocols = { -- these will be put into InputTypes	
        -- this was tested on a live display.The responses are in a different order to the commands and do not match the protocol document.
  		BM = { 
  			...
  			["HDMI 1"       ]={ Tx='\x0E', Rx={ '\x05\x05', '\x05\x03\x03' } },
  			["HDMI 2"       ]={ Tx='\x0F', Rx={ '\x05\x04', '\x05\x03\x04' } },
  			["VGA"          ]={ Tx='\x17', Rx={ '\x05\x01', '\x06\x04\x00' } },
  			["DVI"          ]={ Tx='\x09', Rx={ '\x05\x02' } }, -- RX is a repeat of DP
  			["HDMI"         ]={ Tx='\x08' },
  		},
      DM = {
  			["Display Port" ]={ Tx='\x16', Rx={ '\x05\x01' } },
  			["HDMI 1"       ]={ Tx='\x0E', Rx={ '\x05\x06' } },
  			["HDMI 2"       ]={ Tx='\x0F', Rx={ '\x05\x07' } },
 		},
      DMv1 = { -- this worked in the first installation but not with current models
  			["Menu"         ]={ Tx='\x05\x00' },
      ...
      for i,input in ipairs(InputTypes) do
        if(input.Tx == val)then
          ...
          return i, input.Name
            ...
            --PrintByteString(v, input.Name..' Rx: ')
  					if(v == val)then
          		if DebugFunction then print('GetInputIndex('..i..') InputTypes: '..input.Name) end
          		return i, input.Name
  					end
    ...
    local PropertiesToGet = { "Status", "InputStatus", "MACAddress", "SWVersion", "SerialNumber", "ModelName", "ModelNumber"}
   	if DisplaySeries ~= "GM" then table.insert(PropertiesToGet, "DeviceName") end -- doesn't work on BM series
    ...
    function ClearVariables()
    	...
    	CommandQueue = {}
  		...
  		PropertiesToGet = { "Status", "InputStatus", "MACAddress", "SWVersion", "SerialNumber", "DeviceName", "ModelName", "ModelNumber" }
      for _,v in ipairs(PropertiesToGet) do 
        if Controls[v] and v~="MACAddress" then Controls[v].String = "" end
      end		
	  end
    ...
    function SetActiveInput(index)
    	...
    	if(index)then
    		if Controls["Input"] then Controls["Input"].String = InputTypes[index].Name end
    		if Controls["InputButtons"] then
          if Controls["InputButtons"][ActiveInput] then Controls["InputButtons"][ActiveInput].Value = false end
          if Controls["InputButtons"][index]       then Controls["InputButtons"][index].Value       = true end
        end
    		if Controls["InputStatus"] then 
          if Controls["InputStatus"][ActiveInput] then Controls["InputStatus"][ActiveInput].Value = false end
    		  if Controls["InputStatus"][index]       then Controls["InputStatus"][index].Value       = true end
        end
    		ActiveInput = index
    	else
    		if Controls["Input"] then Controls["Input"].String = "Unknown" end
    		if Controls["InputButtons"] and Controls["InputButtons"][ActiveInput] then Controls["InputButtons"][ActiveInput].Value = false end
    		if Controls["InputStatus"]  and Controls["InputStatus"][ActiveInput]  then Controls["InputStatus"][ActiveInput].Value  = false end
    	end
    end
    ...
    local function GetMSeriesString(ByteString)
      local result = ""
      for i=1,ByteString:len() do
    ...
    if ConnectionType == "Serial" then
    	...
    	function SendNextCommand()
          ...
          if DisplaySeries == "BM" or DisplaySeries == "GM" then command = GetMSeriesString(command) end
    ...
    --  Ethernet Command Function  --DisplaySeries == "BM"
      ...
      local function find_nic_and_broadcast(nics, target_ip)
       	if DebugFunction then print("find_nic_and_broadcast("..target_ip..')') end
        local target_num = IPV4ToValue(target_ip)
       	--if DebugFunction then print("find_nic_and_broadcast, target_num: "..target_num) end
        if not target_num then return nil, nil, "Invalid target IP address" end
        
        for i, nic in ipairs(nics) do
          --for k,v in pairs(nic) do print('['..i..'] '..k..': '..v) end -- '[1] Interface; Ethernet'
          local addr = nic.Address and nic.Address or nic.address
          if addr and #addr>1 then 
            local nic_addr = IPV4ToValue(addr)
            local mask = IPV4ToValue(nic.Netmask and nic.Netmask or nic.mask)
            
            if nic_addr and mask then
              -- Check if target is in same subnet
              local nic_network = nic_addr & mask
              local target_network = target_num & mask
              
              if nic_network == target_network then
                -- Calculate broadcast address
                local broadcast = nic_network | (~mask & 0xFFFFFFFF)
                return i, ValueToIPV4(broadcast), nic
              end
            end
          end  
        end
        return nil, nil, "No matching NIC found for target IP"
      end
      ...
      function SetPowerOn(ctl)
          ...
          -- get nic and broadcast address
          print("Get Broadcast Address for: " .. IPAddress.String)
          local nic_index, broadcast_addr, nic = find_nic_and_broadcast(nics, IPAddress.String)
          local address = nic.Address and nic.Address or nic.address
					if nic_index then
            print("NIC Index: " .. nic_index)
            print("Broadcast Address: " .. broadcast_addr)
            print("Using NIC: " .. address)
            broadcastRange = broadcast_addr
          else
            print("No address calculated, using: " .. broadcastRange)
          end
            ...
            --for k,v in pairs(interface) dop rint('['..name..'] '..k..': '..v) end -- '[1] Interface; Ethernet'
     				if interface.Gateway == "0.0.0.0" then
    					for _,nic in pairs(nics) do
          			local address = nic.Address and nic.Address or nic.address
          			local mask = nic.Netmask and nic.Netmask or nic.mask
    						local ipValue = IPV4ToValue(address)
    						local maskValue = IPV4ToValue(mask or "255.255.255.0")  -- Mask may not be available in emulation mode
    ...
    function ParseResponse(str)
      ...
      if msg then
        if DebugFunction then print("ASCII RX ["..#msg.."]: "..msg) end
    		if DisplaySeries == "Auto" then 
          DisplaySeries = "BM" 
          if DebugTx then print("Setting DisplaySeries fro Auto to "..DisplaySeries) end
        end
      ...
      if #CommandQueue<1 then
        print('Current PollQueue size: '..#PollQueueCurrent)
      ...
      --if DebugRx then PrintByteString(msg.Data, "Data: ") end
        ...
        if Request.ModelName.Data2 == msg.Data2 then
          ...
          if msg['Ack'] then 
            ...
            local series = msg["Data"]:match("%d+(%a+)")
            print('Display series: '..series)
            if Properties["Display Series"].Value == "Auto" then 
              if Properties["Display Series"].Choices[series] then
                print('Setting Auto display series to: '..series)
        ...
        --print("data:byte(1): "..msg["Data"]:byte(1))
    		...
    		--SetActiveInput( GetInputIndex(msg["Data"]:sub(2,3)) ) -- removed because this is inconsistent, InputStatus will tell us what input it is on
    	...
    	--Panel Status
    	elseif msg["Command"]==Request.PanelOn.Command or (Request.PanelStatus and msg["Command"]==Request.PanelStatus.Command) then
    ...
    Controls["PanelOff"].EventHandler = function()
      ...
      if DisplaySeries == "BM" or DisplaySeries == "GM" then
        SetPowerOff()
      end
    end
